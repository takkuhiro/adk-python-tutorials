...
def before_model_modifier(callback_context: CallbackContext, llm_request: LlmRequest) -> types.Content | None:
    ...
    llm_request.contents[-1].parts[0].text = new_user_message  # type: ignore[index]
...
def after_model_modifier(callback_context: CallbackContext, llm_response: LlmResponse) -> types.Content | None:
    ...
    """
    last_agent_response = ""
    if llm_response.content.parts:  # type: ignore[union-attr]
        last_agent_response = llm_response.content.parts[0].text  # type: ignore[union-attr, assignment]
    # ここでメッセージを編集する
    ...
    return new_response  # type: ignore[return-value]
...
root_agent = Agent(
    ...
    before_model_callback=before_model_modifier,  # type: ignore[arg-type]
    after_model_callback=after_model_modifier,  # type: ignore[arg-type]
)
