...
def before_model_modifier(callback_context: CallbackContext, llm_request: LlmRequest) -> Optional[types.Content]:
    """
...
def after_model_modifier(callback_context: CallbackContext, llm_response: LlmResponse) -> Optional[types.Content]:
    """
    ...
    # - return Content: LLMの実行結果を無視してContentをエージェントの応答として出力する（編集して渡したい場合は編集したContentをreturnする）
    new_response = LlmResponse(
        content=types.Content(role="agent", parts=[types.Part(text=new_agent_response)])
    )
    return new_response
